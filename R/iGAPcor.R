...
#' @rdname cor
#' @export
cor.symbolic_tbl <- function(x, method = c("centers", "BD", "BG"), ...) {
  iData <- x
  ...
  cor_matrix <- sapply(1:num_of_variables, function(x) sapply(1:num_of_variables, function(y) cor(interval_data[[x]], interval_data[[y]],method, ...)))
...
#' @rdname cor
#' @export
cor.symbolic_interval <- function(x, y, method = c("centers", "BD", "BG"), ...) {
  if(method == "centers"){
    out <- stats::cor((min(x) + max(x)) / 2, (min(y) + max(y)) / 2)
  }else{
    out <- cov(x, y, method, ...) / (RSDA::sd(x,method = c("billard")) * RSDA::sd(y,method = c("billard")))
  }
  return(out)
}
